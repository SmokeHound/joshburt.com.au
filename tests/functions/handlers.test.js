...
(async () => {
  ...
  try {
    ...
    const register = await authFn.handler(
      makeEvent({
        query: { action: 'register' },
        body: { action: 'register', email, password: 'Passw0rd!', name: 'Handler Test' }
      })
    );
    if (![200, 201, 409].includes(register.statusCode)) {
      console.error('❌ register handler unexpected', register);
      process.exitCode = 1;
      return;
    }
    ...
    const login = await authFn.handler(
      makeEvent({
        query: { action: 'login' },
        body: { action: 'login', email: 'admin@joshburt.com.au', password: 'Admin123!' }
      })
    );
    const loginBody = JSON.parse(login.body || '{}');
    if (login.statusCode !== 200 || !loginBody.accessToken) {
      console.error('❌ login handler failed', login);
      process.exitCode = 1;
      return;
    }
    ...
    const usersNoAuth = await usersFn.handler({
      path: '/.netlify/functions/users',
      httpMethod: 'GET',
      headers: {}
    });
    if (![401, 403].includes(usersNoAuth.statusCode)) {
      console.error('❌ users list should be unauthorized', usersNoAuth);
      process.exitCode = 1;
      return;
    }
    ...
    const usersAuth = await usersFn.handler({
      path: '/.netlify/functions/users',
      httpMethod: 'GET',
      headers: { authorization: `Bearer ${token}` }
    });
    const usersAuthBody = JSON.parse(usersAuth.body || '{}');
    if (usersAuth.statusCode !== 200 || !Array.isArray(usersAuthBody.users)) {
      console.error('❌ users list with token failed', usersAuth);
      process.exitCode = 1;
      return;
    }
