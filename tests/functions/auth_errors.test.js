...
(async () => {
  ...
  try {
    // Wrong password
    const wrong = await postAuth('login', {
      email: 'admin@joshburt.com.au',
      password: 'WRONG-PASS'
    });
    if (wrong.status === 200 || !wrong.json.error) {
      console.error('❌ Expected login failure not observed', wrong);
      process.exitCode = 1;
      return;
    }
    ...
    if (meNoToken.status === 200) {
      console.error('❌ Expected unauthorized for me without token');
      process.exitCode = 1;
      return;
    }
    ...
    // Fake token
    const meFake = await fetch(`${BASE}/.netlify/functions/auth?action=me`, {
      headers: { Authorization: 'Bearer faketoken' }
    });
    if (meFake.status === 200) {
      console.error('❌ Expected failure for fake token');
      process.exitCode = 1;
      return;
    }
    ...
    console.error('❌ Auth error tests exception', e);
    process.exitCode = 1;
  }
})();
