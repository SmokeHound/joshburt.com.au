...
exports.handler = withHandler(async function(event){
  ...
  async function handleGet(event) {
    try {
      ...
      if (search) {
        conditions.push('(part_code LIKE ? OR part_name LIKE ? OR supplier LIKE ? OR inv_number LIKE ?)');
  ...
  async function handlePost(event) {
    try {
      ...
      if (!return_date || !supplier || !part_code || !part_name || !quantity || !unit_price || !return_reason || !status) {
        return error(400, 'Missing required fields: return_date, supplier, part_code, part_name, quantity, unit_price, return_reason, status');
      }
      
      // Validate status
      const validStatuses = ['pending_shipment', 'in_transit', 'received_by_supplier', 'under_review', 'approved', 'credited', 'rejected'];
      if (!validStatuses.includes(status)) {
        return error(400, 'Invalid status value');
      }
      
      // Validate return reason
      const validReasons = ['defective', 'wrong_part', 'warranty', 'excess_stock', 'customer_return', 'other'];
      ...
      const created = await database.get('SELECT * FROM supplier_returns WHERE id = ?', [result.id]);
  ...
  async function handlePut(event) {
    try {
      ...
      if (!id || !return_date || !supplier || !part_code || !part_name || !quantity || !unit_price || !return_reason || !status) {
        return error(400, 'Missing required fields: id, return_date, supplier, part_code, part_name, quantity, unit_price, return_reason, status');
      }
  ...
  async function handleDelete(event) {
    try {
      ...
      const existing = await database.get('SELECT part_code, supplier FROM supplier_returns WHERE id = ?', [id]);
