...
async function verifyAuth0Token(token) {
  try {
    ...
    if (alg !== 'RS256') {
      return null;
    } // Auth0 tokens are RS256 by default
...
async function getOrCreateUserFromClaims(claims) {
  try {
    ...
    if (!email) {
      return null;
    } // require email mapping for local account

    let user = await database.get(
      'SELECT id, email, name, role, is_active, email_verified FROM users WHERE email = ?',
      [email]
    );
    ...
    const autoProvision =
      (process.env.AUTH0_AUTO_PROVISION || autoProvisionDefault).toLowerCase() === 'true';
    if (!user && autoProvision) {
      ...
      const result = await database.run(
        'INSERT INTO users (email, name, role, email_verified, is_active) VALUES (?, ?, ?, ?, ?)',
        [email, name, 'user', verified, 1]
      );
    ...
    if (decodedLocal && decodedLocal.userId) {
      try {
        const user = await database.get(
          'SELECT id, email, name, role, is_active, email_verified, created_at, updated_at, last_login, avatar_url, totp_enabled FROM users WHERE id = ?',
          [decodedLocal.userId]
        );
        if (!user || !user.is_active) {
          return null;
...
async function requireAuth(event, roles = null) {
  ...
  if (!user) {
    return { user: null, response: error(401, 'Authentication required') };
  }
  if (roles && !roles.includes(user.role)) {
    return { user, response: error(403, 'Insufficient permissions') };
  }
...
async function requirePermission(event, resource, action) {
  ...
  if (authResponse) {
    return { user, response: authResponse };
  }
