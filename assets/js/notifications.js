...
(function () {
  ...
  const unread = new Set(JSON.parse(localStorage.getItem('notificationUnread') || '[]'));
  let settings = JSON.parse(localStorage.getItem('notificationSettings') || '{}');
  if (!settings || typeof settings !== 'object') {
    settings = {};
  }
  if (!Array.isArray(settings.mutedTypes)) {
    settings.mutedTypes = [];
  }
  ...
  function persist() {
    prune();
    localStorage.setItem('siteNotifications', JSON.stringify(notifications.slice(0, 300)));
    localStorage.setItem('notificationId', String(notificationId));
    localStorage.setItem('notificationUnread', JSON.stringify([...unread]));
  ...
  function spawnToast(n) {
    ...
    toast.innerHTML = `<div class="text-lg leading-none">${meta.icon}</div><div class="flex-1"><div class="font-medium">${escapeHtml(n.message)}</div><div class="text-xs opacity-75">Just now</div></div><button aria-label="Dismiss" class="font-bold ml-2" style="line-height:1;">&times;</button>`;
  ...
  window.showNotification = function (message, type = 'info', duration = 5000) {
    ...
    const n = { id, message, type, timestamp: Date.now() };
  ...
  function group(list) {
    ...
    list.forEach(n => {
      if (n.timestamp >= startToday) {
        g.today.push(n);
      } else if (n.timestamp >= startYesterday) {
        g.yesterday.push(n);
      } else {
        g.older.push(n);
      }
  ...
  function render() {
    ...
    function renderGroup(label, arr) {
      ...
      return (
        `<div class='px-3 pt-3 text-[10px] uppercase tracking-wide opacity-60'>${label}</div>` +
        arr
          .map(n => {
            const meta = TYPE_META[n.type] || TYPE_META.info;
            const isUnread = unread.has(n.id);
            return (
              '<div tabindex="0" class="notification-item flex gap-3 p-3 rounded outline-none focus:ring-2 focus:ring-secondary hover:bg-gray-100 dark:hover:bg-gray-700 group ' +
              ...
              meta.icon +
              '</div>' +
              '<div class="flex-1 min-w-0"><div class="text-sm ' +
              (isUnread ? 'font-semibold' : 'font-medium') +
              '">' +
              escapeHtml(n.message) +
              '</div><div class="text-[10px] uppercase tracking-wide opacity-60 mt-1">' +
              relativeTime(n.timestamp) +
              '</div></div>' +
              '<button class="text-[10px] text-blue-400 hover:underline toggle-read" aria-label="Toggle read state">' +
    ...
    const muteTypesHtml = ['info', 'success', 'warning', 'error']
      .map(function (t) {
        const disabled = t === 'error' ? 'disabled' : '';
        const checked = settings.mutedTypes.includes(t) ? 'checked' : '';
        const extra =
          t === 'error' ? "<span class='text-[10px] opacity-60'>(cannot mute)</span>" : '';
        return (
          "<label class='flex items-center gap-2'><input type='checkbox' class='notif-mute-type' data-type='" +
          t +
          "' " +
          checked +
          ' ' +
    ...
    listEl.querySelectorAll('.toggle-read').forEach(btn =>
    ...
    listEl.querySelectorAll('.notification-item').forEach(item =>
      item.addEventListener('keydown', function (e) {
        if (e.key === 'Enter' || e.key === ' ') {
          ...
          const next =
            e.key === 'ArrowDown'
              ? itemsArr[idx + 1] || itemsArr[0]
              : itemsArr[idx - 1] || itemsArr[itemsArr.length - 1];
          if (next) {
            next.focus();
          }
    ...
    save &&
      save.addEventListener('click', e => {
        ...
        settings.mutedTypes = [
          ...document.querySelectorAll('#notification-settings-panel .notif-mute-type:checked')
        ]
      ...
      document.addEventListener('keydown', function (e) {
        ...
        if (!dropdown.classList.contains('hidden') && e.key === 'Tab') {
          const focusables = [].slice.call(
            dropdown.querySelectorAll('button,[tabindex="0"],input,select')
          );
