...
(function() {
  ...
  function applyCSSVariables(colors) {
    ...
    // Spacing scale (overrides spacing tokens)
    ...
    const spacingScale = parseScale(colors.spacingScale);
    if (spacingScale !== null) {
      ...
      root.style.setProperty('--token-spacing-xs', `${Math.round(base.xs * spacingScale)}px`);
      root.style.setProperty('--token-spacing-sm', `${Math.round(base.sm * spacingScale)}px`);
      root.style.setProperty('--token-spacing-md', `${Math.round(base.md * spacingScale)}px`);
      root.style.setProperty('--token-spacing-lg', `${Math.round(base.lg * spacingScale)}px`);
      root.style.setProperty('--token-spacing-xl', `${Math.round(base.xl * spacingScale)}px`);
      root.style.setProperty('--token-spacing-2xl', `${Math.round(base.x2l * spacingScale)}px`);
    } else {
      root.style.removeProperty('--token-spacing-xs');
      root.style.removeProperty('--token-spacing-sm');
      root.style.removeProperty('--token-spacing-md');
      root.style.removeProperty('--token-spacing-lg');
      root.style.removeProperty('--token-spacing-xl');
      root.style.removeProperty('--token-spacing-2xl');
    }

    // Font scale + base weight
    const fontScale = parseScale(colors.fontScale);
    if (fontScale !== null) {
      root.style.setProperty('--token-font-scale', String(fontScale));
    } else {
      root.style.removeProperty('--token-font-scale');
    }

    const baseWeight = (() => {
      if (colors.baseFontWeight === null || colors.baseFontWeight === undefined) {
        return null;
      }
      const n = parseInt(String(colors.baseFontWeight).trim(), 10);
      if (!Number.isFinite(n)) {
        return null;
      }
    ...
    if (baseWeight !== null) {
      root.style.setProperty('--token-font-weight-normal', String(baseWeight));
      root.style.setProperty('--token-font-weight-medium', String(Math.min(900, baseWeight + 100)));
      root.style.setProperty('--token-font-weight-semibold', String(Math.min(900, baseWeight + 200)));
      root.style.setProperty('--token-font-weight-bold', String(Math.min(900, baseWeight + 300)));
    } else {
      root.style.removeProperty('--token-font-weight-normal');
      root.style.removeProperty('--token-font-weight-medium');
      root.style.removeProperty('--token-font-weight-semibold');
      root.style.removeProperty('--token-font-weight-bold');
    }

    // Apply font scale to document (kept separate from tokens for safety)
  ...
  function applyTheme(themeId, customColors) {
    ...
    const colors = {
      ...
      spacingScale: (customColors && customColors.spacingScale) || null,
      fontScale: (customColors && customColors.fontScale) || null,
      baseFontWeight: (customColors && customColors.baseFontWeight) || null
    };
  ...
  const ThemeManager = {
    ...
    applyFromStorage: function() {
      ...
      const hasCustomColors = [
        ...
        storedColors.spacingScale,
        storedColors.fontScale,
        storedColors.baseFontWeight
    ...
    setPalette: function(colors, persist) {
      ...
      if (persist !== false) {
        try {
          ...
          if (colors.spacingScale !== undefined && colors.spacingScale !== null) {
            siteSettings.spacingScale = colors.spacingScale;
          }
          if (colors.fontScale !== undefined && colors.fontScale !== null) {
            siteSettings.fontScale = colors.fontScale;
          }
          if (colors.baseFontWeight !== undefined && colors.baseFontWeight !== null) {
            siteSettings.baseFontWeight = colors.baseFontWeight;
          }
          localStorage.setItem('siteSettings', JSON.stringify(siteSettings));
          ...
          if (colors.spacingScale !== undefined && colors.spacingScale !== null) {
            localStorage.setItem('spacingScale', String(colors.spacingScale));
          }
          if (colors.fontScale !== undefined && colors.fontScale !== null) {
            localStorage.setItem('fontScale', String(colors.fontScale));
          }
          if (colors.baseFontWeight !== undefined && colors.baseFontWeight !== null) {
            localStorage.setItem('baseFontWeight', String(colors.baseFontWeight));
          }
        } catch (e) {
          // Ignore storage errors
        }
    ...
    getActiveTheme: function() {
      ...
      return {
        ...
        colors: {
          ...
          spacingScale: storedColors.spacingScale || null,
          fontScale: storedColors.fontScale || null,
          baseFontWeight: storedColors.baseFontWeight || null
        }
      };
    },
